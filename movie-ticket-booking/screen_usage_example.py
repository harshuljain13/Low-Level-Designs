...
def demonstrate_basic_screen_operations():
    ...
    print(f"After booking - Available: {screen.available_count}, Occupied: {screen.occupied_count}")
...
def demonstrate_custom_layout():
    ...
    for row_idx, row in enumerate(seat_map, 1):
        print(f"Row {row_idx:2d}: {' '.join(seat[:12] if seat else '____' for seat in row)}")
    print()
...
def demonstrate_imax_screen():
    ...
    for row_idx, row in enumerate(seat_map[:5], 1):
        seat_ids = [seat[-6:] if seat else '______' for seat in row]  # Show last 6 chars
...
def demonstrate_concurrent_booking_safety():
            ...
            try:
                ...
                if available_seats:
                    ...
                    if success:
                        ...
                        print(f"Worker {worker_id}: Failed to book {seat.seat_id} (already booked)")
    ...
    print(f"Final state - Available: {screen.available_count}, Occupied: {screen.occupied_count}")
    print()
...
def demonstrate_pricing_with_screen_context():
    ...
    for strategy_name, strategy in pricing_strategies:
        ...
        print(f"{strategy_name:8} - Premium: ₹{premium_price:6.2f}, Recliner: ₹{recliner_price:6.2f}")
