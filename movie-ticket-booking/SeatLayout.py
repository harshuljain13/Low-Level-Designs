...
class SeatLayout:
    ...
    def add_seat(self, seat: Seat, row: int, col: int) -> None:
        if row < 0 or row >= self._num_rows or col < 0 or col >= self.seats_per_row[row]:
...
if __name__ == "__main__":
    ...
    seat_layout.add_seat(SeatFactory.create_seat(SeatType.PREMIUM, "TEST001", "S1", base_price=100.0), row=1, col=1)
    seat_layout.add_seat(SeatFactory.create_seat(SeatType.REGULAR, "TEST002", "S2", base_price=100.0), row=2, col=2)
