...
class Screen(ABC):
    ...
    """
    def __init__(self, screen_id: str, screen_name: str, screen_type: str, 
                 screen_capacity: int, num_rows: int, seats_per_row: List[int]):
    ...
    def prepare_seat_layout(self) -> None:
        ...
        for row in range(self._num_rows//2):
            for col in range(self._seats_per_row[row]):
                self._seat_layout.add_seat(PremiumSeat(f"R{row}C{col}",
                    f"R{row}C{col}", 200), row, col)

        for row in range(self._num_rows//2, self._num_rows):
            for col in range(self._seats_per_row[row]):
                self._seat_layout.add_seat(RegularSeat(f"R{row}C{col}",
                f"R{row}C{col}", 100), row, col)
...
class ThreeD(Screen):
    ...
    def prepare_seat_layout(self) -> None:
        ...
        for row in range(self._num_rows//2, self._num_rows):
            for col in range(self._seats_per_row[row]):
                self._seat_layout.add_seat(ReclinerSeat(f"R{row}C{col}",
                    f"R{row}C{col}", 100), row, col)
